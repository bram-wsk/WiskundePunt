  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      {(activeThemeId || activeSubModuleGroupId) && (
        <div className="bg-white p-8 rounded-[3rem] border border-blue-50 shadow-sm relative overflow-hidden mb-8">
            <div className="flex items-center gap-3 mb-4">
               <button onClick={handleBack} className="text-blue-600 font-black uppercase text-[10px] tracking-widest bg-blue-50 px-4 py-2 rounded-xl border-none cursor-pointer hover:bg-blue-100 transition-all">
                  <i className="fa-solid fa-arrow-left mr-2"></i> Terug
               </button>
               {activeTheme && (
                 <>
        ...
        {modulesToDisplay.map((item) => {
          ...
          if (item.id === 'mix' && !activeThemeId) {
            return (
              <div 
                ...
                className={`group relative p-8 rounded-[3rem] shadow-xl transition-all flex flex-col h-80 overflow-hidden border-none bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 cursor-pointer hover:shadow-[0_20px_60px_-15px_rgba(99,102,241,0.5)]`}
              >
                <div className="absolute top-0 right-0 p-8 opacity-20 scale-150 rotate-12 transition-transform duration-700 group-hover:rotate-0 pointer-events-none">
                  <span className={`text-9xl font-black font-serif leading-none select-none text-white`}>Σ</span>
                </div>
                
                <div className="relative z-10 space-y-6 flex-1">
                  <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-inner border bg-white/20 border-white/10 text-white`}>
                    <span className="text-4xl font-black font-serif pb-1">Σ</span>
                  </div>
                  <div>
                    <h3 className={`text-xl font-black text-white`}>{item.title}</h3>
                    <p className={`text-white/80 text-sm font-bold mt-2 leading-relaxed line-clamp-2`}>{item.description}</p>
                  </div>
                </div>

                <div className={`relative z-10 mt-6 pt-6 border-t flex items-center justify-between shrink-0 border-white/20`}>
                  <span className={`text-[10px] font-black uppercase tracking-widest text-white/60`}>
                    Adaptief
                  </span>
                  <div className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg transform transition-transform bg-white text-indigo-600 group-hover:scale-105`}>
                    Starten
                  </div>
                </div>
              </div>
          ...
          return (
            <div 
              key={item.id}
              onClick={() => handleSelectModule(item)}
              className={`group bg-white p-8 rounded-[3rem] border-2 transition-all shadow-lg flex flex-col h-80 relative overflow-hidden border-transparent hover:border-blue-500 hover:shadow-2xl cursor-pointer`}
            >
              <div className={`space-y-6 flex-1`}>
                <div className="flex justify-between items-start">
                  <div className={`w-16 h-16 ${iconBgClass} ${iconTextColorClass} rounded-[1.2rem] flex items-center justify-center text-2xl transition-transform group-hover:rotate-12 shadow-sm`}>
                    {item.icon.startsWith('fa-') ? (
                      <i className={`fa-solid ${item.icon}`}></i>
                    ) : (
                      <span className="font-serif font-black text-3xl">{item.icon}</span>
                    )}
                  </div>
                  {(!item.subModules || item.subModules.length === 0) && ( // Only show level for leaf nodes
                    <div className={`px-4 py-2 ${level.bg} ${level.color} rounded-full text-[9px] font-black uppercase tracking-widest flex items-center gap-2 border border-current/10`}>
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900 group-hover:text-blue-600 transition-colors">{item.title}</h3>
                  <p className="text-slate-400 text-xs font-medium mt-1 leading-relaxed line-clamp-3">{item.description}</p>
                </div>
              </div>
              ...
              <div className={`mt-6 pt-6 border-t border-slate-50 flex items-center justify-between shrink-0`}>
                ...
                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all bg-slate-50 text-slate-300 group-hover:bg-blue-600 group-hover:text-white`}>
                  <i className={`fa-solid ${(!item.subModules || item.subModules.length === 0) ? 'fa-play text-xs' : 'fa-arrow-right'}`}></i>
                </div>
              </div>
            </div>
          );
